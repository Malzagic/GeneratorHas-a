class PasswordGenerator {
  constructor(){
    ...
    this.lengthCheckbox = document.querySelector('#length');
  ...
  init() {
    document.querySelectorAll('.options input[type="checkbox"]').forEach(checkbox => checkbox.addEventListener('click', this.updateOptions));
  ...
  getRandomUppercase() { // otrzymamy wartości z zakresu 
                        // od 65 do 90
  ...
  generatePassword = () => {
    if(!this.lengthCheckbox.value) return;
    if(this.optionMethods.length === 0) return;

    const arrIndexes = Array.from(Array(+this.lengthCheckbox.value).keys())
